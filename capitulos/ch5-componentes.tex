...
O pacote Application segue o padrão de Camada de Serviço~\cite{buschmann2007service}, nesse caso, uma subcamada. E contém subpacotes relacionados à logica, como os pacotes de casos de uso (UseCases), de mapeamento (Mappers) e condicionais baseadas em regras de negócio (Strategies).
...
\hl{i.ex.:} Na primeira versão do domínio, a relação era entre \verb|BanhistaTosador| e \verb|Servico|, impedindo um funcionário do tipo \verb|Veterinario| de ser associado a um serviço.
Outro exemplo, foi a entidade \verb|EstoqueProduto|, que foi renomeada para \verb|EstoqueProdutoFilial|, para representar melhor a relação entre as filiais e os produtos, onde cada filial possui uma quantidade em estoque de cada produto.
Além disso, as classes \verb|Pessoa| e \verb|Usuario| foram adicionadas como superclasses para as entidades \verb|Cliente| e \verb|Funcionario|, respectivamente, para evitar a duplicação de atributos comuns entre essas entidades.
...
\section{Camada de Interface com o Usuário}
\label{sec-ciu}

A Camada de Interface com o Usuário (CIU) é responsável por fornecer a interação entre os usuários finais e o sistema, apresentando as informações de forma clara e intuitiva, além de capturar as ações e comandos dos usuários para serem processados pela lógica de negócio.

Essa camada é composta por componentes que gerenciam a apresentação visual (views) e o controle de acionamento de fluxos através dos controllers, que por sua vez fazem a comunicação com a CLN através das Classes Gerenciadoras de Tarefas (CGTs) contidas no pacote Application (\ref{sec-componentes-cln-application}).

% \vitor{Adicionar imagens da IU (prints)}

Com base na interface definida nos protótipos, a CIU foi projetada separando os componentes de interface (views) dos componentes de controle (controllers), que interagem com os componentes da CLN para realizar as operações solicitadas pelos usuários.
A seguir, são representados os principais fluxos da CIU:

% \begin{figure}[h]
% 	\centering
% 	\includegraphics[width=1\textwidth]{figuras/diagrama_ciu.png}
% 	\caption{Camada de Interface com o Usuário (views e controllers) separada em subpacotes}
% 	\label{figura-cius}
% \end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth,height=0.3\textheight]{figuras/ciu_cliente.png}
	\caption{Fluxo de cadastro e gerenciamento de clientes}
	\label{figura-ciu-cliente}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1.0\textwidth,height=0.5\textheight]{figuras/ciu_agendamentos_filiais.png}
	\caption{Fluxo de cadastro e gerenciamento de agendamentos e filiais}
	\label{figura-ciu-agendamentos-filiais}
\end{figure}

\pedro{Na Figura~\ref{figura-ciu-agendamentos-filiais}, as setas entre views e controllers deveriam ser bidirecionais porém, devido limitações técnicas, as setas possuem apenas uma direção}
